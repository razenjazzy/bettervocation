...
class CourseApiController extends Controller
{
            ...
            if (isModuleActive('OrgSubscription') && auth('api')->check()) {
                if (!orgSubscriptionCourseSequence($id)) {
                    $response = [
                        'success' => false,
                        'data' => $course,
                        'message' => 'You Can Not Continue This . Pls Complete Previous Course',
                    ];
                    return response()->json($response, 200);
                }
            }
    ...
    public function buyNow(Request $request)
    {
        try {
            ...
            $carts = Cart::where('user_id', auth('api')->user()->id)->with('course', 'course.user')->get();
            $total = Cart::where('user_id', auth('api')->user()->id)->sum('price');
            if ($total == 0) {
