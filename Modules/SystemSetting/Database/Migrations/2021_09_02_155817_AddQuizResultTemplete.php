...
class AddQuizResultTemplete extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        $template = EmailTemplate::where('act', 'QUIZ_RESULT_TEMPLATE')->first();
        if (!$template) {
            $template = new EmailTemplate();
            $template->act = 'QUIZ_RESULT_TEMPLATE';
        }
        $shortCode = '{"quiz":"Quiz Name","mark":"Mark","total":"Total Mark","status":"Status"}';
        $subject = 'Quiz Result';

        $body = "{{quiz}} result published. You {{status}} the exam. Your obtain Marks is {{mark}}/{{total}}. {{footer}}";
