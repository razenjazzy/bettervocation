...
class LanguageController extends Controller
{
    ...
    public function get_translate_file(Request $request)
    {
        try {
            ...
            $file_name = explode('.', $request->file_name)[0] ?? '';
            $translatable_file_name = $request->file_name;

            if ($file_name == 'permission') {
                $languages = $this->syncPermission();
            } else {
...
//                $languages = include "{$url}";
    ...
    public function syncPermission()
    {
        ...
        try {
            ...
            $file = resource_path('lang/default/permission.php');

            foreach (array_unique($permissions) as $permission) {
                $lang[$permission] = $permission;
            }
            file_put_contents($file, '<?php return ' . var_export($lang, true) . ';');
            return $lang;
        } catch (\Exception $e) {
            return $lang;
        }
