...
class AddFooterStaticPage extends Migration
{
    ...
    public function up()
    {
        ...
        $bettervocation = FrontPage::where('slug', 'BetterVocation-for-business')->first();
        if (!$bettervocation) {
            DB::table('front_pages')->insert([
                'name' => 'BetterVocation for Business',
                'title' => 'BetterVocation for Business',
                'sub_title' => 'BetterVocation for Business',
                ...
                'slug' => 'bettervocation-for-business',
                'status' => 1,
                'is_static' => 0,
            ]);
            $bettervocation = FrontPage::where('slug', 'bettervocationcation-for-business')->first();

        }
        $check6 = FooterWidget::where('slug', 'bettervocation-for-business')->first();
        if ($check6) {
            $check6->slug = $bettervocation->slug;
            $check6->page_id = $bettervocation->id;
            $check6->is_static = $bettervocation->is_static;
            $check6->save();
        }

        $teach = FrontPage::where('slug', 'teach-on-bettervocation')->first();
        if (!$teach) {
            DB::table('front_pages')->insert([
                'name' => 'Teach on BetterVocation',
                'title' => 'Teach on BetterVocation',
                'sub_title' => 'Teach on BetterVocation',
        ...
        $check7 = FooterWidget::where('slug', 'teach-on-bettervocation')->first();
        if ($check7) {
